...
5. **Initialize results.tsv**: Create `results.tsv` with just the header row. The baseline will be recorded after the first run. The file `results.tsv` must NEVER be committed.
...
- Change the experiment surface in `EXPERIMENT` and the helper builders: model family, `prior_estimator`, covariance/mu estimators, `risk_measure`, `objective_function`, pipelines, constraints, long/short limits, L1/L2, etc and all other things discussed in the original https://skfolio.org documentation or using your skfolio skill.
...
- Add explicit friction controls such as `transaction_costs`, `management_fees`, `max_turnover`, and `l1_coef` only when supported by the optimizer.
- Adjust the validation suite in `train.py` only when you are making it more robust or more representative, not easier.

**What you CANNOT do:**

- Change validation to inflate the score.
...
- model selection by means of constantly seeded `MultipleRandomizedCV` in each experimental validation
...
When an experiment is done, log it to `results.tsv` as a single tab-separated line (do NOT use commas in descriptions — only tabs).

The `results.tsv` file always keeps all output on a single line per experiment, with these columns (header row first):

```
commit val_sharpe seconds failed_cases robust_pass description
```
...
Example (header and two lines):

```
commit  val_sharpe  seconds  failed_cases  robust_pass  description
a1b2c3d	0.968529	3.9	0	True	baseline run
b2c3d4e	0.000000	0.0	0	False	crash: model OOM
```

The script prints a `results_tsv_row:` line for easy and consistent copying — always in a single line, no wrapped fields or extraneous whitespace.

The script prints a `results_tsv_row:` line to make this logging easier and more consistent.

## The experiment loop

Run on a dedicated branch (e.g. `autoresearch/mar14`).
...
As an example use case, a user might leave you running while they sleep. If each experiment takes you ~10 seconds then you can run approx 360 experiments hour, for a total of about 2880 over the duration of the average human sleep. The user then wakes up to experimental results, all completed by you while they slept!
